<template>
  <div>
    <h1 class="text-xl font-bold text-gray-800 mb-2">Skill 管理</h1>
    <p class="text-gray-500 text-sm mb-4">管理员可创建、编辑、隐藏或删除 Skill。隐藏后公开列表不可见。</p>

    <el-dialog v-model="editing" :title="editId ? '编辑 Skill' : '新建 Skill'" width="700" draggable class="admin-dialog">
      <el-form :model="form" label-position="top" @submit.prevent="saveSkill">
        ...
        <el-form-item label="Markdown 文档">
          <el-input v-model="form.contentMd" type="textarea" :rows="12" placeholder="SKILL.md 等 Markdown 文档内容，详情页将渲染展示" class="font-mono text-sm" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <div class="flex gap-3 mb-4">
      <el-button type="primary" @click="startNew">新建 Skill</el-button>
...
const form = ref({ name: '', description: '', tagsStr: '', cloneCommand: '', contentMd: '' })
...
function startNew() {
  editId.value = null
  form.value = { name: '', description: '', tagsStr: '', cloneCommand: '', contentMd: '' }
  editing.value = true
  error.value = ''
}
