<template>
  <div>
    <h1 class="text-xl font-bold text-gray-800 mb-2">MCP 服务器管理</h1>
    <div class="flex gap-3 mb-4">
      ...
      <el-select v-model="selectedCategory" placeholder="标签" clearable class="w-40">
        <el-option label="全部标签" value="" />
        <el-option v-for="opt in categories" :key="opt" :label="opt" :value="opt" />
      </el-select>
      <el-button type="primary" @click="load">搜索</el-button>
      <router-link to="/mcp" class="el-button el-button--default">查看公开列表</router-link>
    </div>
    ...
    <el-table :data="items" stripe>
      ...
      <el-table-column prop="name" label="名称" min-width="150">
        <template #default="{ row }">
          <router-link :to="'/mcp/' + row.id" class="text-primary hover:underline">{{ row.name }}</router-link>
        </template>
      </el-table-column>
...
async function load() {
  ...
  try {
    ...
    const data = await api.get('/admin/mcp', { params })
...
async function loadCategories() {
  try {
    categories.value = await api.get('/mcp/filter-options')
  } catch (_) {
    categories.value = []
  }
}
...
async function fetchStatus() {
  ...
  try {
    statusData.value = await api.get('/admin/scrape-status/mcp')
...
async function triggerScrape() {
  ...
  error.value = ''
  try {
    await api.post('/admin/scrape/mcp')
...
async function remove(row) {
  try {
    await ElMessageBox.confirm('确定删除该 MCP 服务器？', '确认', {
      type: 'warning',
    })
